...
namespace CG.CqrsCrud.Generators
{
    public static class AddMediatorGenerator<T>
    {
        public static bool GenerateAddMediator(string plural, string commandNameSpace, string commandPath)
        {
            ...
            File.WriteAllLines($"{commandPath}\\Add{typeof(T).Name}Command.Validator.cs", ValidationGenerator<T>
                .Generate(plural, commandNameSpace, "Add", typeof(T).GetProperties().Where(x=>x.SetMethod != null && !x.GetCustomAttributes().Where(a=>a is AutoGeneratedPrimaryKey).Any()).ToList()));
            return true;
        }
    }
}
